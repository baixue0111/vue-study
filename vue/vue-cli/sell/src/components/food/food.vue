    ...
    <div v-show="showFlag" class="food" ref="food">
      <div class="food-content">
        ...
        <div class="rating">
          ...
          <ratingselect v-bind:select-type="selectType" v-bind:only-content="onlyContent" v-bind:desc="desc" v-bind:ratings="food.ratings" v-on:selecttype="setType" v-on:content="setOnlycontent"></ratingselect>
  ...
  export default {
    ...
    methods: {
      ...
      addFirst(event) {
        ...
        this.$emit('cartEvent', event.target);  //  子组通过$emit触发父组件cartAdd方法
        Vue.set(this.food, 'count', 1);  // 设置对象的属性
      },
      needShow(type, text) {
    ...
    filters: {
      formatDate(time) {
        let date = new Date(time);  // 定义一个date对象
