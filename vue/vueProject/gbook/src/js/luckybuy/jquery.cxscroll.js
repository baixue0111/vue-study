...
(function($){
	$.fn.cxScroll=function(settings){
		...
		scroller.items=scroller.list.find(".liscroll").each();
		scroller.itemSum=scroller.items.length;

		/*	console.log('scroller.list.find(".liscroll")='+scroller.list.find(".liscroll")[0]);*/
		...
		scroller.fn.goto=function(d){
			...
			case "top":
				_max=0;
				if(d=="left"){
					if(parseInt(scroller.box.scrollLeft(),2)==0){
					...
					scroller.box.animate({"scrollLeft":_dis},settings.speed,function(){
						if(parseInt(scroller.box.scrollLeft(),2)<=_max){
							scroller.box.scrollLeft(0);
						};
						scroller.fn.addControl();
					});
				}else{
					if(parseInt(scroller.box.scrollTop(),2)==0){
					...
					scroller.box.animate({"scrollTop":_dis},settings.speed,function(){
						if(parseInt(scroller.box.scrollTop(),2)<=_max){
